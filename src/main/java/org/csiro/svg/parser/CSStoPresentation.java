 ...
 *****************************************************************************/
//
// $Id: CSStoPresentation.java,v 1.2 2000/09/26 14:10:37 dino Exp $
//
...
public class CSStoPresentation {
	...
	try {
	    ...
	    SVGDocument svgDoc = parser.parseSVG( args[0]);
	    ...
	    outF.setDoctype(OutputFormat.whichDoctypePublic(svgDoc),
			    OutputFormat.whichDoctypeSystem(svgDoc));
    ...
    public static void convertStyle( org.w3c.dom.Node node) {
	...
	if (node instanceof SVGStylable ) {
		...
		for (int i=0; i < styleDec.getLength(); i++) {
		    ...
		    ((org.w3c.dom.Element)node).setAttribute( att, val);
		}

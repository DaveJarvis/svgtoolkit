 ...
 *****************************************************************************/
//
// $Id: SVGTransformListImpl.java,v 1.20 2000/11/21 05:57:07 bella Exp $
//
...
/**
 * SVGTransformListImpl is the implementation of org.w3c.dom.svg.SVGTransformList
 */
public class SVGTransformListImpl extends SVGListImpl implements
  SVGTransformList {
   ...
   * equivalent transformation matrices together to result in a list consisting
   * of a single SVGTransform object of type SVG_TRANSFORM_MATRIX.
   *
   * @return The resulting SVGTransform object which becomes single item in
   * the list. If the list was empty, then a value of null is returned.
   */
  public SVGTransform consolidate() {
    int numTransforms = (int)getNumberOfItems();
    if (numTransforms == 0) {
      return null;
    }
    SVGTransform transform = (SVGTransform)getItem(0);
    SVGMatrix result = transform.getMatrix();
    for (int i = 1; i < numTransforms; i++) {
      transform = (SVGTransform)getItem(i);
  ...
  /**
   * Parses the transform string and returns an SVGTransformList that represents
   * the transforms.
   * @param transformString The string that contains the transformation commands.
    ...
    while (st.hasMoreTokens()) {
      ...
      if (transformType.equals("matrix")) {
        ...
        if (numArgs == 1) {
          ...
          System.out.println("wrong number of args for translate transform: translate(" + transformArgs + ")");
          if (numArgs > 2) {
            tx = Float.parseFloat(st1.nextToken());
            ty = Float.parseFloat(st1.nextToken());
          }
  ...
  public AffineTransform getAffineTransform() {
    ...
    AffineTransform affineTransform = new AffineTransform(result.getA(), result.getB(),
                            result.getC(), result.getD(), result.getE(), result.getF());

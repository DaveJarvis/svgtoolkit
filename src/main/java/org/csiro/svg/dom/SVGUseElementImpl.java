 ...
 *****************************************************************************/
//
// $Id: SVGUseElementImpl.java,v 1.38 2002/02/21 07:46:09 brett Exp $
//
...
public class SVGUseElementImpl extends SVGGraphic implements SVGUseElement, Drawable {
  ...
  /**
   * Simple constructor, dimensions initialized to: x=0, y=0, width=100%, height=100%.
   */
   ...
   */
  public SVGUseElementImpl(SVGDocumentImpl owner, org.w3c.dom.Element elem) {
   ...
   * objects should be recreated. Set this to true if the use has changed
   * in any way since the last time it was drawn. Otherwise set to false for
   * speedier rendering.
   *
   * Note: at the moement the use element ignores the refreshData parameter as
   * it seemed to be causing incorrect rendering.
   */
  public void draw(Graphics2D graphics, boolean refreshData) {
    ...
    //          <svg x="useX" y="useY" width="useWidth" height="useHeight" viewBox="symbolViewBox"
    ...
    //     <use>
    //       <g transform="useTransform">
    //         <svg x="useX" y="useY" width="useWidth" height="useHeight" viewBox="0,0,useWidth,useHeight">
    //           <clone of referencedElement style="original refenced element's complete style (including parents)">
    //         </svg>
    //       <g>
    //     <use>
  ...
  //  if (refreshData || clonedRef == null) {
      ...
      if (href.length() > 0) {
        ...
        if (index != -1) {
          ...
          // If this is an external reference then set ref to null to prevent
          // infinite referencing loops.
          if (index > 0) {
            String docName = getOwnerDoc().getURL().toLowerCase();
            String refDocName = href.substring(0, index).toLowerCase();
            if ( !docName.contains( refDocName ) )  {
               ref = null;
               System.out.println("Use element error: External reference '" + href + "' not supported.");
               return;
            }
          }
       }

        if ( (ref instanceof Drawable || ref instanceof SVGSymbolElement)
          && ref instanceof SVGStylable ) {

          //  System.out.println("found reference element for href: " + href);
          clonedRef = ((SVGElementImpl)ref).cloneElement();
        } else {
          if (ref == null) {
            System.out.println("Use element error: Couldn't find element that matches reference '" + href + "'");
            return;
          } else {
  ...
  private void drawShape(Graphics2D graphics, boolean refreshData) {
    ...
    if (clonedRef instanceof SVGSymbolElement ) {   // is a symbol
      ...
      if (clonedRef.hasChildNodes()) {
        org.w3c.dom.NodeList children = clonedRef.getChildNodes();
      ...
      ((SVGAnimatedPreserveAspectRatioImpl)svgElement.getPreserveAspectRatio()).setBaseVal(((SVGSymbolElement)clonedRef).getPreserveAspectRatio().getBaseVal());
      if (((SVGSymbolElement)clonedRef).getViewBox().getAnimVal().getWidth() > 0) {
        svgElement.setAttribute("viewBox", ((SVGRectImpl)((SVGSymbolElement)clonedRef).getViewBox().getBaseVal()).toString());
      } else {
        svgElement.setAttribute("viewBox",   "0,0,"+ getWidth().getAnimVal().getValue()
                                               + "," + getHeight().getAnimVal().getValue());
      }
      //draw symbol

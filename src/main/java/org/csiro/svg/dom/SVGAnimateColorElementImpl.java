 ...
 *****************************************************************************/
//
// $Id: SVGAnimateColorElementImpl.java,v 1.7 2000/11/30 03:08:29 bella Exp $
//
...
public class SVGAnimateColorElementImpl extends SVGAnimationElementImpl implements
    ...
    if (getAttribute("values").length() > 0) {
      ...
      if (beforeSVGColor != null && afterSVGColor != null) {
        ...
        if (calcMode.equals("linear") || calcMode.equals("paced") || calcMode.equals("spline")) {
          ...
          int red = (int)(beforeColor.getRed() + percentBetween  * (afterColor.getRed() - beforeColor.getRed()));
          int green = (int)(beforeColor.getGreen() + percentBetween  * (afterColor.getGreen() - beforeColor.getGreen()));
          int blue = (int)(beforeColor.getBlue() + percentBetween  * (afterColor.getBlue() - beforeColor.getBlue()));
          return "rgb(" + red + "," + green + "," + blue + ")";

        } else if (calcMode.equals("discrete")) {
          if (percentBetween < 1) {
            Color beforeColor = beforeSVGColor.getColor();
            return "rgb(" + beforeColor.getRed() + "," + beforeColor.getGreen() + "," + beforeColor.getBlue() + ")";
          } else {
            Color afterColor = afterSVGColor.getColor();
      ...
      if (from.length() > 0 && to.length() > 0) { // is a from-to anim
        ...
        int green = (int)(fromColor.getGreen() + percentageComplete * (toColor.getGreen() - fromColor.getGreen()));

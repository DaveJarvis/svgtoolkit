// SVGPathSegCurvetoCubicSmoothRelImpl.java
//
/*****************************************************************************
 ...
 *****************************************************************************/
//
// $Id: SVGPathSegCurvetoCubicSmoothRelImpl.java,v 1.5 2000/11/21 05:57:03 bella Exp $
//
...
public class SVGPathSegCurvetoCubicSmoothRelImpl extends SVGPathSegImpl implements
  ...
  public String toString() {
    return getPathSegTypeAsLetter() + " " + getX2() + " " + getY2() + " "
                + getX() + " " + getY();
  }
   /**
   * Returns the length of this path segment.
   * @param startPoint The starting point for this segment.
   * @param lastControlPoint Of previous curve, used to determine the first control point
   */
  public float getTotalLength( SVGPoint startPoint, SVGPoint lastControlPoint) {
    ...
    for (int i=1; i < steps; i++) {
       // Calculate length of each segment
        SVGPoint point1 = (SVGPoint) Points.getItem(i-1);
        SVGPoint point2 = (SVGPoint) Points.getItem(i);
        double seglength =
  ...
  public SVGPoint getPointAtLength(float distance, SVGPoint startPoint, SVGPoint lastControlPoint) {
    ...
    templength = templength - seglength[right];
      //  double ratio = (distance-seglength[left])/seglength[right];
    double ratio = Math.abs(distance-templength)/seglength[right];

    SVGPoint rightPoint = (SVGPoint) Points.getItem(right);
    SVGPoint leftPoint = (SVGPoint) Points.getItem(left);

    double newX = 0.0, newY = 0.0;
    // Test if at end of curve then return end points otherwise formulas return infinity
    if (Math.abs(distance - length) > 0.001) {
      ...
      newX = getX() + startPoint.getX();
      newY = getY() + startPoint.getY();
    }
  ...
  private void calculatePoints(SVGPoint startPoint, SVGPoint lastControlPoint) {
    ...
    float controlPoint1X = 2*startPoint.getX() - (float)lastControlPoint.getX();
    float controlPoint1Y = 2*startPoint.getY() - (float)lastControlPoint.getY();

    float startPointX = startPoint.getX();
    float startPointY = startPoint.getY();
    float controlPoint2X = getX2() + startPointX;
    float controlPoint2Y = getY2() + startPointY;
    float endPointX = getX() + startPointX;
    float endPointY = getY() + startPointY;

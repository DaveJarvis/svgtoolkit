 ...
 *****************************************************************************/
//
// $Id: SVGAnimateElementImpl.java,v 1.14 2000/11/30 03:08:29 bella Exp $
//
...
public class SVGAnimateElementImpl extends SVGAnimationElementImpl implements
  SVGAnimateElement {
  ...
  private SVGLength getCurrentLengthValue() {
    ...
    if (getAttribute("values").length() > 0) {
      ...
      for (int i = 0; i < times.size()-1; i++) {
        ...
        //  System.out.println("checking for between: " + beforeTime + " and " + afterTime + " percent complete = " + percentageComplete);
        if (percentageComplete >= beforeTime && percentageComplete <= afterTime) {
          beforeLength = new SVGLengthImpl((String)vals.elementAt(i), getTargetElement(), SVGLengthImpl.NO_DIRECTION);
          afterLength = new SVGLengthImpl((String)vals.elementAt(i+1), getTargetElement(), SVGLengthImpl.NO_DIRECTION);
          //   System.out.println("time between " + i + " and " + (i+1));
          break;
        }
        if (i == (times.size()-2) && calcMode.equals("discrete") && percentageComplete > afterTime) {
          beforeLength = new SVGLengthImpl((String)vals.elementAt(i+1), getTargetElement(), SVGLengthImpl.NO_DIRECTION);
          afterLength = new SVGLengthImpl((String)vals.elementAt(i+1), getTargetElement(), SVGLengthImpl.NO_DIRECTION);
          //   System.out.println("time between " + i + " and " + (i+1));
          break;
        }
      ...
      if (beforeLength != null && afterLength != null) {
        ...
        if (calcMode.equals("linear") || calcMode.equals("paced")) {
          float lengthVal = beforeLength.getValue() + percentBetween * (afterLength.getValue() - beforeLength.getValue());
    ...
    // either a from-to, from-by, by or to animation
    ...
    } else if (getAttribute("from").length() > 0
            || getAttribute("to").length() > 0
      ...
      if (from.length() > 0 && to.length() > 0) { // is a from-to or to anim
        ...
        return new SVGLengthImpl(fromValue + percentageComplete * (toValue - fromValue), getTargetElement(), SVGLengthImpl.NO_DIRECTION);

      } else if (from.length() > 0 && by.length() > 0) { // is a from-by or to anim
        SVGLengthImpl fromLength = new SVGLengthImpl(from, getTargetElement(), SVGLengthImpl.NO_DIRECTION);
        SVGLengthImpl byLength =  new SVGLengthImpl(by, getTargetElement(), SVGLengthImpl.NO_DIRECTION);
        float fromValue = fromLength.getValue();
        float byValue = byLength.getValue();
        return new SVGLengthImpl(fromValue + percentageComplete*byValue, getTargetElement(), SVGLengthImpl.NO_DIRECTION);
      }
    }
    return null;
  }
    ...
    if (getAttribute("values").length() > 0) {
        ...
        if (calcMode.equals("linear") || calcMode.equals("paced") || calcMode.equals("spline")) {
          if (beforeLengthList.getNumberOfItems() == afterLengthList.getNumberOfItems()) {
            ...
            for (int i = 0; i < numItems; i++) {
              float beforeValue = ((SVGLength)beforeLengthList.getItem(i)).getValue();
              float afterValue = ((SVGLength)afterLengthList.getItem(i)).getValue();
              currentList.appendItem(new SVGLengthImpl(beforeValue + percentBetween * (afterValue - beforeValue), getTargetElement(), SVGLengthImpl.NO_DIRECTION));
            }
            return currentList;
          } else {
      ...
      if (from.length() > 0 && to.length() > 0) { // is a from-to anim
        ...
        if (fromLengthList.getNumberOfItems() == toLengthList.getNumberOfItems()) {
          ...
          for (int i = 0; i < numItems; i++) {
            float fromValue = ((SVGLength)fromLengthList.getItem(i)).getValue();
            float toValue = ((SVGLength)toLengthList.getItem(i)).getValue();
            currentList.appendItem(new SVGLengthImpl(fromValue + percentageComplete * (toValue - fromValue), getTargetElement(), SVGLengthImpl.NO_DIRECTION));
          }
          return currentList;
        } else {
      ...
      } else if (from.length() > 0 && by.length() > 0) { // is a from-by anim
        SVGLengthList fromLengthList = makeLengthList(from);
        ...
        if (fromLengthList.getNumberOfItems() == byLengthList.getNumberOfItems()) {
          ...
          for (int i = 0; i < numItems; i++) {
            float fromValue = ((SVGLength)fromLengthList.getItem(i)).getValue();
            float byValue = ((SVGLength)byLengthList.getItem(i)).getValue();
            currentList.appendItem(new SVGLengthImpl(fromValue + percentageComplete * byValue, getTargetElement(), SVGLengthImpl.NO_DIRECTION));
          }
          return currentList;
        } else {
  ...
  private String getCurrentStringValue() {
    ...
    if (getAttribute("from").length() > 0 && getAttribute("to").length() > 0) {
      ...
      // if (currentTime > start) return from, if (currentTime > end) return to
  ...
  private Float getCurrentNumberValue() {
    ...
    if (getAttribute("values").length() > 0) {
      ...
      if (beforeNumber != null && afterNumber != null) {
        float percentBetween = (percentageComplete - beforeTime)/(afterTime-beforeTime);
        if (calcMode.equals("linear") || calcMode.equals("paced")) {
          return new Float(beforeNumber.floatValue() + percentBetween * (afterNumber.floatValue() - beforeNumber.floatValue()));
        ...
        if (calcMode.equals("linear") || calcMode.equals("paced") || calcMode.equals("spline")) {
          if (beforeNumberList.getNumberOfItems() == afterNumberList.getNumberOfItems()) {
            ...
            for (int i = 0; i < numItems; i++) {
              ...
              currentList.appendItem(new SVGNumberImpl(beforeValue + percentBetween * (afterValue - beforeValue)));
            }
            return currentList;
          } else {
      ...
      if (from.length() > 0 && to.length() > 0) { // is a from-to anim
        ...
        if (fromNumberList.getNumberOfItems() == toNumberList.getNumberOfItems()) {
          ...
          for (int i = 0; i < numItems; i++) {
            ...
            currentList.appendItem(new SVGNumberImpl(fromValue + percentageComplete * (toValue - fromValue)));
          }
          return currentList;
        } else {
          System.out.println("cannot animate length list, from and to lists have different number of items");
  ...
  private SVGAngle getCurrentAngleValue() {
    ...
    if (getAttribute("values").length() > 0) {
      ...
      if (beforeAngle != null && afterAngle != null) {
        float percentBetween = (percentageComplete - beforeTime)/(afterTime-beforeTime);
        if (calcMode.equals("linear") || calcMode.equals("paced")) {
          float angleVal = beforeAngle.getValue() + percentBetween * (afterAngle.getValue() - beforeAngle.getValue());

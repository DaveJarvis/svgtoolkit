 ...
 *****************************************************************************/
//
// $Id: SVGDocumentImpl.java,v 1.45 2002/02/21 07:46:09 brett Exp $
//
...
import org.csiro.svg.dom.events.*;
 ...
 */
public class SVGDocumentImpl extends DocumentImpl implements SVGDocument, DocumentView {
  ...
  private boolean animated = false; // indicates whether document contains any animations
   ...
   */
	...
	//
	// Embed this document into the script engine as a variable called "document".
	//
	public void exposeToScriptEngine(String variableName, Object theObject)
	{
    System.out.println("SVGDocumentImpl about to expose '" + variableName + "' to script engine as " + theObject.toString());
    Scriptable jsArgs = getContext().toObject(theObject,getScope());
  ...
  public SVGDocumentImpl( org.w3c.dom.Document doc) {
    initialiseScriptEngine();
    ...
    if (rootElement.getElementsByTagName("script").getLength() > 0) {
      // only expose to script engine if doc contains some scripting stuff
       exposeToScriptEngine("document", this);
        //exposeToScriptEngine("timer", new Timer(new org.csiro.svg.dom.events.ScriptTimerListener(this)));
  ...
  /**
   * Returns the title of a document as specified by the title sub-element of the
   * 'svg' root element (i.e., <svg><title>Here is the title</title>...</svg>)
   */
        ...
        for (int i = 0; i < numChildren; i++) {
          ...
          if (child instanceof SVGTitleElement ) {
            ...
            if (title.hasChildNodes()) {
              org.w3c.dom.NodeList titleChildren = title.getChildNodes();
              int numTitleChildren = titleChildren.getLength();
              for (int j = 0; j < numTitleChildren; j++) {
                org.w3c.dom.Node titleChild = titleChildren.item( j);
                if (titleChild.getNodeType() == org.w3c.dom.Node.TEXT_NODE) { // it is #PCDATA
  ...
  public void setTitle( String title ) {
    ...
    if (rootElement != null) {
      ...
      org.w3c.dom.Node titleTextNode = createTextNode( title);
  ...
  public void draw(Graphics2D graphics) {
    if (!beginTimeSet) {
      // this is the first draw, set the docBeginTime, this will be used when
    ...
    SVGSVGElementImpl rootElement = (SVGSVGElementImpl)getRootElement();
    if (rootElement != null) {
     // System.out.println(rootElement.getCurrentTime() + " : " + count++);
  ...
  public Element createElement( String tag)
    ...
    if (tag == null) {
      throw new org.w3c.dom.DOMException( org.w3c.dom.DOMException.INVALID_CHARACTER_ERR, "createElement failed, invalid tag name");
    }
    if (tag.equals("svg")) {
  ...
  private Vector getAnimationElements() {
    ...
    org.w3c.dom.NodeList animateElements = getRootElement().getElementsByTagName( "animate");
    ...
    org.w3c.dom.NodeList animateTransformElements = getRootElement().getElementsByTagName( "animateTransform");
    ...
    org.w3c.dom.NodeList setElements = getRootElement().getElementsByTagName( "set");
    int numSetElements = setElements.getLength();
    for (int i = 0; i < numSetElements; i++) {
      animationElements.addElement(setElements.item(i));
    }

    org.w3c.dom.NodeList animateColorElements = getRootElement().getElementsByTagName( "animateColor");
    ...
    org.w3c.dom.NodeList animateMotionElements = getRootElement().getElementsByTagName( "animateMotion");

 ...
 *****************************************************************************/
//
// $Id: SVGPathSegArcAbsImpl.java,v 1.5 2000/11/21 05:30:38 cath Exp $
//
...
public class SVGPathSegArcAbsImpl extends SVGPathSegImpl implements
  ...
  public String toString() {
    ...
    if (getLargeArcFlag())
      largeArc = 1;
    return getPathSegTypeAsLetter() + " " + getR1() + " " + getR2() + " " + getAngle() + " "
                + largeArc + " " + sweep + " " + getX() + " " + getY();
  }
  ...
  public SVGPoint getPointAtLength(float distance, SVGPoint startPoint, SVGPoint lastControlPoint) {
    ...
    if (Math.abs(distance-getTotalLength(startPoint,lastControlPoint)) < 0.001)
      ...
      // Need to convert from endpoint to centre parameterization to find extent angle
      double cosAngle = Math.cos(angle);
      double sinAngle = Math.sin(angle);
      ...
      double startAngle = Math.acos(
        (ux*vx + uy*vy) / (Math.sqrt(Math.abs(ux*ux + uy*uy)) * Math.sqrt(Math.abs(vx*vx + vy*vy))));
      ...
      double extentAngle = 2*distance /
      (r1+r2)*(1 + (3*(Math.pow((r1-r2)/(r1+r2),2)))/(10 + Math.sqrt(3-4*(Math.pow((r1-r2)/(r1+r2),2)))));
      ...
      double deltaAngle = extentAngle - startAngle;
      ...
      double tempX = Math.cos(angle)*r1*Math.cos(startAngle+extentAngle)
                  - Math.sin(angle)*r2*Math.sin(startAngle+extentAngle)
                  + cx ;

      double tempY = Math.sin(angle)*r1*Math.cos(startAngle+extentAngle)
                   + Math.cos(angle)*r2*Math.sin(startAngle+extentAngle)
                   + cy;

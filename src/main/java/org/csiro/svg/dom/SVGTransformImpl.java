 ...
 *****************************************************************************/
//
// $Id: SVGTransformImpl.java,v 1.12 2000/09/26 14:10:35 dino Exp $
//
...
public class SVGTransformImpl implements SVGTransform {
  ...
  /**
   * Returns the type of the value as specified by one of the SVGTransform constants.
  ...
  /**
   * Returns the matrix that represents this transformation.
   * For SVG_TRANSFORM_MATRIX, the matrix contains the a, b, c, d, e, f values
   * supplied by the user.
   * For SVG_TRANSFORM_TRANSLATE, e and f represents the translation amounts
   * (a=1,b=0,c=0,d=1).
   * For SVG_TRANSFORM_SCALE, a and d represents the scale amounts (b=0,c=0,e=0,f=0).
   * For SVG_TRANSFORM_ROTATE, SVG_TRANSFORM_SKEWX and SVG_TRANSFORM_SKEWY,
   * a, b, c and d represent the matrix which will result in the given
   * transformation (e=0,f=0).
   ...
   * For SVG_TRANSFORM_MATRIX, SVG_TRANSFORM_TRANSLATE and SVG_TRANSFORM_SCALE,
   * angle will be zero.
  ...
  /**
   * Sets the transform type to SVG_TRANSFORM_TRANSLATE, with parameters tx and
   * ty defining the translation amounts.
  ...
  public String toString() {
    switch (type) {
      case SVG_TRANSFORM_MATRIX: return "matrix(" + getFloatString(matrix.getA())
                                            + " " + getFloatString(matrix.getB())
                                            + " " + getFloatString(matrix.getC())
                                            + " " + getFloatString(matrix.getD())
                                            + " " + getFloatString(matrix.getE())
                                            + " " + getFloatString(matrix.getF()) + ")";
      case SVG_TRANSFORM_ROTATE: {
        if (rotateOrigin.getX() == 0 && rotateOrigin.getY() == 0) {
          return "rotate(" + getFloatString(angle) + ")";
        } else {
          return "rotate(" + getFloatString(angle)
                     + " " + getFloatString(rotateOrigin.getX())
      ...
      case SVG_TRANSFORM_SKEWX: return "skewX(" + getFloatString(angle) + ")";
      case SVG_TRANSFORM_SKEWY: return "skewY(" + getFloatString(angle) + ")";
      case SVG_TRANSFORM_TRANSLATE: return "translate(" + getFloatString(matrix.getE()) + " "
                                                        + getFloatString(matrix.getF()) + ")";
      default: return "";
    }
  }

 ...
 *****************************************************************************/
//
// $Id: SVGLengthImpl.java,v 1.22 2002/02/21 07:46:09 brett Exp $
//
...
public class SVGLengthImpl implements SVGLength {
   ...
   * @param v  Value to assign to valueInSpecifiedUnits.
   */
   ...
   * @param cssText  The value to assign to valueAsString, eg. "10" or "10cm".
   */
  public void setValueAsString(String cssText) throws DOMException {
    ...
    // Handle exponent value setting if present
    if (index != -1 && (text.charAt(index) == 'e' || text.charAt(index) == 'E')) {
      // Determine whether this is in fact an exponent value, which should be of the form: 'E' ['-'|'+'] d+
      String testExponent = text.substring(index);
      // Find first non-exponent setting character after the initial E
      int expIndex = -1;
      for (int i=1; i<testExponent.length(); i++) {
        if ( !Character.isDigit(testExponent.charAt(i)) && (testExponent.charAt(i) != '-') && (testExponent.charAt(i) != '+') ) {
          expIndex = i;
          break;
        }
      }

      // Get the integer value for the exponent setting
      String testExponentValue;
      if (expIndex != -1)
        testExponentValue = testExponent.substring(1,expIndex);
      else
        testExponentValue = testExponent.substring(1);
      try {
        exponentValue = (float)Math.pow(10,Integer.parseInt(testExponentValue));

        // Now we have to look for a unit value again, having dealt with the exponent setting
        index = -1;
        text = text.substring(0,index) + text.substring(index + testExponentValue.length() + 1, text.length());
        for (int i = 0; i < text.length(); i++) {
          if (!Character.isDigit(text.charAt(i)) && text.charAt(i) != '.'
               && text.charAt(i) != '-' && text.charAt(i) != '+') {
            index = i;
        ...
        // Mustn't be an exponent setting after all, but a unit specification
        exponentValue = 1;
      }
    ...
    if (index == -1) {  // no unit specified
      unitType = SVG_LENGTHTYPE_NUMBER;
      setValue(Float.parseFloat(text) * exponentValue);

    } else {  // there was a unit

      String unit = text.substring(index);
      unitType = getUnitTypeConst(unit);
      valueInSpecifiedUnits = Float.parseFloat(text.substring(0,index)) * exponentValue;
    }
  }
  ...
  /**
   * Checks if the given unit type is a valid one. If not, throws an SVGException.
   */
  private void checkUnitType(short unitType) throws SVGException {
    if (! (unitType == SVG_LENGTHTYPE_CM
  ...
  private float convertToUserUnits(float value, short unitType) {
    ...
    if (ownerSVGElement != null) {
      if (lengthDir == X_DIRECTION) {
        ...
        pixelToMM = (ownerSVGElement.getScreenPixelToMillimeterX() + ownerSVGElement.getScreenPixelToMillimeterY())/2;
      }
    }
  ...
  float getTransformedLength(AffineTransform transform) {
    ...
    if (unitType == SVG_LENGTHTYPE_NUMBER || transform == null
  ...
  private static String getUnitText(short unit) {
    switch (unit) {
      case SVG_LENGTHTYPE_NUMBER: return "";
      case SVG_LENGTHTYPE_CM: return "cm";
      case SVG_LENGTHTYPE_EMS: return "em";
      case SVG_LENGTHTYPE_EXS: return "ex";
      case SVG_LENGTHTYPE_IN: return "in";
      case SVG_LENGTHTYPE_MM: return "mm";
      case SVG_LENGTHTYPE_PC: return "pc";
      case SVG_LENGTHTYPE_PERCENTAGE: return "%";
      case SVG_LENGTHTYPE_PT: return "pt";
      case SVG_LENGTHTYPE_PX: return "px";
      default: return "";
    }
  }
  ...
  private static short getUnitTypeConst(String unit) {
    if (unit == null) {
      return SVG_LENGTHTYPE_NUMBER;  // assume that the unit wasn't specified
    }

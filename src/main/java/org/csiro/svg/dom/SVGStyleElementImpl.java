 ...
 *****************************************************************************/
//
// $Id: SVGStyleElementImpl.java,v 1.9 2000/09/26 14:10:34 dino Exp $
//
...
public class SVGStyleElementImpl extends SVGElementImpl implements
  SVGStyleElement {
  ...
  public String getStyleText() {
    String styleText = "";
    if (hasChildNodes()) {
      org.w3c.dom.NodeList styleChildren = getChildNodes();
      int numStyleChildren = styleChildren.getLength();
      for (int j = 0; j < numStyleChildren; j++) {
        org.w3c.dom.Node styleChild = styleChildren.item( j);
        if ((styleChild.getNodeType() == org.w3c.dom.Node.CDATA_SECTION_NODE)
             || (styleChild.getNodeType() == org.w3c.dom.Node.TEXT_NODE)) {
  ...
  private void setupStyles() {
      ...
      if (st.hasMoreTokens()) {
        ...
        if (styleDecl != null) {
          ...
          while(st2.hasMoreTokens()) {
            ...
            if (token.indexOf("[") == 0) {
              // it is an attribute selector so it may contain commas
              while (token.indexOf("]") == -1 && st2.hasMoreTokens()) {
                // append adjacent tokens till find the end bracket
                token += "," + st2.nextToken();
              }
            }
   ...
   // this function needs to be a lot more complicated to cope with all possible
  // style selectors
  private boolean selectorMatchesAncestorElem(String selector, SVGStylableImpl element) {
    ...
    if (token != null) {
      ...
      if (token.startsWith(".")) {
       // is a class selector
        ...
        if (tag.equals(selectorTag)) {
          // now make sure that any classnames listed are in the element's class list
      ...
      } else if (matched && numTokens > 1) {    // see if parent matches the rest of the selector bit

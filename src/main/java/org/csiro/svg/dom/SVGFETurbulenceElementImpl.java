 ...
 *****************************************************************************/
//
// $Id: SVGFETurbulenceElementImpl.java,v 1.3 2000/09/26 14:10:22 dino Exp $
//
...
public class SVGFETurbulenceElementImpl extends SVGFilterPrimitive implements
...
//  	      int rgba;
//  	      int r, g, b, a;
//  	      r = (int) (turbulence(0, point, 0.1, 0.05, 6, false, false, 0, 0, 0, 0) * 255);
//  	      g = (int) (turbulence(1, point, 0.05, 0.1, 4, false, false, 0, 0, 0, 0) * 255);
//  	      b = (int) (turbulence(2, point, 0.1, 0.05, 2, false, false, 0, 0, 0, 0) * 255);
//  	      a = (int) (turbulence(3, point, 0.05, 0.1, 8, false, false, 0, 0, 0, 0) * 255);
//  	      rgba = (a << 24) | (r << 16) | (g << 8) | b;
//  	      resultImage.setRGB((int) imageSpaceBounds.getX() + i, (int) imageSpaceBounds.getY() + j, rgba);
//  	  }
//        }
    ...
    */
	...
	i = 0;
		...
		s = (double)(Math.sqrt(fGradient[k][i][0] * fGradient[k][i][0] + fGradient[k][i][1] * fGradient[k][i][1]));
		fGradient[k][i][0] /= s;
		fGradient[k][i][1] /= s;
	    }
        }
    ...
    private double turbulence(int nColorChannel, double[] point, double fBaseFreqX, 
			      double fBaseFreqY, int nNumOctaves, boolean bFractalSum, 
			      boolean bDoStitching, double fTileX, double fTileY, 
        ...
        if(bDoStitching)
        {
                ...
                double fLoFreq = (double)(Math.floor(fTileWidth * fBaseFreqX)) / fTileWidth;
                double fHiFreq = (double)(Math.ceil(fTileWidth * fBaseFreqX)) / fTileWidth;
		    ...
		    fBaseFreqX = fHiFreq;
                ...
                fLoFreq = (double)(Math.floor(fTileHeight * fBaseFreqX)) / fTileHeight;
                fHiFreq = (double)(Math.ceil(fTileHeight * fBaseFreqX)) / fTileHeight;
		...
		fSum += (double)(Math.abs(noise2(nColorChannel, vec, pStitchInfo)) / ratio);

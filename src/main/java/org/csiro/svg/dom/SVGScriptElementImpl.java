 ...
 *****************************************************************************/
//
// $Id: SVGScriptElementImpl.java,v 1.9 2001/05/09 01:34:50 bella Exp $
//
 ...
 */
public class SVGScriptElementImpl extends SVGElementImpl implements
  SVGScriptElement {
  ...
  public String getScript() {
    String scriptText = "";
    if (hasChildNodes()) {
      org.w3c.dom.NodeList scriptChildren = getChildNodes();
      int numScriptChildren = scriptChildren.getLength();
      for (int j = 0; j < numScriptChildren; j++) {
        org.w3c.dom.Node scriptChild = scriptChildren.item( j);
        if ((scriptChild.getNodeType() == org.w3c.dom.Node.CDATA_SECTION_NODE)
             || (scriptChild.getNodeType() == org.w3c.dom.Node.TEXT_NODE)) {
  ...
  void registerScript(SVGDocumentImpl document, String commandString) {
    //System.err.println("About to register script: " + commandString);
    Object result = null;
    try {
      result = document.getContext().evaluateString(document.getScope(), commandString, "<cmd>", 1, null);
      //System.out.println(document.getContext().toString(result));
    }

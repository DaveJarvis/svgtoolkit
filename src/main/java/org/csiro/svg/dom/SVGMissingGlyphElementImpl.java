// SVGMissingGlyphElementImpl.java
//
/*****************************************************************************
 ...
 *****************************************************************************/
//
// $Id: SVGMissingGlyphElementImpl.java,v 1.10 2000/11/30 05:37:39 bella Exp $
//
...
public class SVGMissingGlyphElementImpl extends SVGStylableImpl implements SVGMissingGlyphElement {
  ...
  public SVGMissingGlyphElementImpl(SVGDocumentImpl owner) {
    super(owner, "missing-glyph");
  }

  public SVGMissingGlyphElementImpl(SVGDocumentImpl owner, Element elem) {
    super(owner, elem, "missing-glyph");
  }
  ...
  SVGMissingGlyphElementImpl clonedGlyph = null;
  float fontHeight = 10;

  public void drawGlyph(Graphics2D graphics, SVGElementImpl parent, float x, float y,
                          float fontSize, boolean refreshData) {

    //  System.out.println("drawing glyph for char: " + getAttribute("unicode"));

    // creates temporary svg structure that looks like:
    //
    //   <parent>
    //     <g transform="translate(x,y)">
    //       <svg width="fontSizeWidth" height="fontSizeHeight" viewBox="fontViewBox" style="parent font's style">
    //         <g2 transform="scale(1,-1)" style="glyph's style">    // need to flip glyph
    ...
    if (refreshData || clonedGlyph == null) {
      ...
      clonedGlyph = (SVGMissingGlyphElementImpl)cloneElement();
      fontHeight = ((SVGFontElementImpl)getParentNode()).getFontAscent();
      compositeShape = null;
    }
    ...
    if (clonedGlyph != null) {
      ...
      if (getAttribute("d") != "") {
        ...
        org.w3c.dom.Node oldParent = getParentNode();
      ...
      if (hasChildNodes()) {
        ...
        if (clonedGlyph.hasChildNodes()) {
          org.w3c.dom.NodeList children = clonedGlyph.getChildNodes();
        ...
        svgElement.setAttribute("style","overflow: visible"); // this tells the svgElement NOT to clip to its viewport
        ((SVGAnimatedLengthImpl)svgElement.getWidth()).setBaseVal(new SVGLengthImpl(width, svgElement, SVGLengthImpl.X_DIRECTION));
        ((SVGAnimatedLengthImpl)svgElement.getHeight()).setBaseVal(new SVGLengthImpl(height, svgElement, SVGLengthImpl.X_DIRECTION));

        svgElement.setAttribute("viewBox", "0, 0, " + fontHeight + ", " + fontHeight);

	       // do actual drawing
	      // refreshData needs to be true because the glyph children may now have different parents
   ...
   */
  public void drawOutline(Graphics2D graphics, float x, float y,
			  float fontSize ) {

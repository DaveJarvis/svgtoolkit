 ...
 *****************************************************************************/
//
// $Id: SVGAnimationElementImpl.java,v 1.10 2000/11/30 03:08:30 bella Exp $
//
...
public abstract class SVGAnimationElementImpl extends SVGElementImpl implements
  SVGAnimationElement {
  ...
  protected boolean active = false;    // indicates animation is currently running
  protected boolean finished = false;  // indicated animation has completely finished, including repeats
  ...
  public float getSimpleDuration() throws org.w3c.dom.DOMException {
    ...
    if (duration < 0) {
      throw new org.w3c.dom.DOMException( org.w3c.dom.DOMException.NOT_SUPPORTED_ERR,
  ...
  protected float getClockSecs(String clockVal) {
    ...
    try {
      if (clockVal.indexOf(":") != -1) {    // is either a full/partial clock value
  ...
  public abstract Object getCurrentValue(short animtype);
   ...
   protected float getNumRepeats(float duration) {
    ...
    if (repeatCount.length() > 0 || repeatDur.length() > 0) { // should maybe repeat
  ...
  protected float checkStatus(float currentTime, float startTime, float duration,
                                      float numRepeats, boolean repeatForever) {
    ...
    if (currentTime < startTime) {   // animation has not started yet
      ...
      && (repeatForever || (currentTime < (startTime + duration*numRepeats)))) {
      ...
      if (getAttribute("fill").equalsIgnoreCase("remove")) {
        return -1;  // this will indicate to use the baseVal as the animVal
  ...
  protected void setupTimeValueVectors(String calcMode, String values) {
    ...
    if (keyTimes.length() == 0) {
      ...
      if (calcMode.equalsIgnoreCase("paced")) {     // paced, only look at the first and last values
        StringTokenizer stVals = new StringTokenizer(values, ";");
        int numVals = stVals.countTokens();
        float currTime = 0;
        int currentTokenCount = 0;
        while (stVals.hasMoreTokens()) {
          if (currentTokenCount == 0 || currentTokenCount == numVals -1) { // first or last value

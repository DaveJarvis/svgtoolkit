 ...
 *****************************************************************************/
//
// $Id: SVGAnimateMotionElementImpl.java,v 1.7 2001/01/29 23:10:28 cath Exp $
//
...
public class SVGAnimateMotionElementImpl extends SVGAnimationElementImpl implements
   ...
   */
  public SVGAnimateMotionElementImpl(SVGDocumentImpl owner, org.w3c.dom.Element elem) {
  ...
  public SVGTransformList getCurrentMotionTransform() {
   ...
   // System.out.println("percentage Complete: " + percentageComplete + " " + currentTime);
    if (getAttribute("values").length() > 0) {
     ...
     //   System.out.println("beforeTransform: " + beforeTransform + " afterTransform " + afterTransform);
  ...
  private SVGTransformList getCurrentTransform(float percentageComplete, float nextPercent, String from, String to, String by) {

  //  System.out.println("CurrentTransform: " + percentageComplete + " " + to + " " + from);
    if (getAttribute("path").length() > 0) {
    ...
    } else if (from.length() >0 || to.length() >0 || by.length() > 0) { // may be to/from or values.

      if (pathElem == null || getAttribute("values").length() > 0) {  // only recalculate if it is a "values" anim
        if (from.length() == 0) {
          from = "0,0";
        }
        ...
        if (to.length() > 0) { // it is a from-to animation
          ...
          String pathString = "M" + fromX.getValue() + "," + fromY.getValue()
                            + "L" + toX.getValue() + "," + toY.getValue();
      ...
      if (pathElem == null) {
        org.w3c.dom.NodeList children = getElementsByTagName( "mpath");
    ...
    if (pathElem != null) {
      ...
      if (getAttribute("rotate").length() > 0) {
        ...
        // Need test in case the point is last point, so gradiant is calculated using previous point.
        SVGPoint nextPoint;
        if (nextPercent >= 1) {
          nextPoint = point;
          point = pathElem.getPointAtLength((float)0.99*distance);
        }
        else // gradiant is calculated using next point.
          nextPoint = pathElem.getPointAtLength(nextPercent*length);

        float gradient = 0;
        if (rotation.equals("auto")) {
          if (((nextPoint.getY() - point.getY()) != 0) && ((nextPoint.getX() - point.getX()) != 0)) {
            gradient = (nextPoint.getY() - point.getY())/(nextPoint.getX() - point.getX());
            rotationAngle = Math.atan((float)gradient);
        ...
        } else if (rotation.equals("auto-reverse")) { // rotation angle equals auto angle + 180
          if (((nextPoint.getY() - point.getY()) == 0) || ((nextPoint.getX() - point.getX()) == 0))
            gradient = 0;
          else
            gradient = (nextPoint.getY() - point.getY())/(nextPoint.getX() - point.getX());

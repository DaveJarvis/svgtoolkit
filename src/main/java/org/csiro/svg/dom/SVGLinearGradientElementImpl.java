 ...
 *****************************************************************************/
//
// $Id: SVGLinearGradientElementImpl.java,v 1.32 2000/12/07 06:51:41 bella Exp $
//
...
/**
 * SVGLinearGradientElementImpl is the implementation of org.w3c.dom.svg.SVGLinearGradientElement
 */
public class SVGLinearGradientElementImpl extends SVGGradientElementImpl implements
  ...
  /**
   * Simple constructor, x1, y1, x2 ande y2 are initialized to default
   * values: 0%, 0%, 100% and 100%
   */
  ...
  public Paint getPaint(SVGElementImpl element, float opacity) {
    ...
    if (element instanceof SVGTransformable ) {
     ...
     // System.out.println("shape bounds = " + ((SVGRectImpl)objectBounds).toString());
    } else {
      objectBounds = vBox;
    }
  ...
  //  System.out.println("Gradient coords = " + x1 + "," + y1 + "," + x2 + "," + y2);


    // if stop points do not go from 0 to 100%, then adjust x1,y1,x2,y2 to be at first and last stop points
   ...
   //  System.out.println("new gadient coords = " + x1 + "," + y1 + "," + x2 + "," + y2);
    ...
    BufferedImage  image = new BufferedImage(maxX-minX, maxY-minY,
    ...
    for (int x = minX; x < maxX; x++) {
      for (int y = minY; y < maxY; y++) {
        ...
        if (reversePercentAlongLine > 1) {
          if (animSpreadMethod == this.SVG_SPREADMETHOD_REPEAT) {
            percentAlongLine = percentAlongLine + 1;
            if (percentAlongLine < 0) {
              percentAlongLine += 1;
            }
          } else if (animSpreadMethod == this.SVG_SPREADMETHOD_REFLECT) {
            percentAlongLine = -percentAlongLine; // this should make it positive
          } else {  // pad
             percentAlongLine = 0;
          }
        }
    ...
    SVGTexturePaint paint = new SVGTexturePaint(image, new Rectangle2D.Float(minX, minY, maxX-minX, maxY-minY), transform,
                                                SVGTexturePaint.SVG_TEXTURETYPE_LINEAR_GRADIENT);
    return paint;
  }

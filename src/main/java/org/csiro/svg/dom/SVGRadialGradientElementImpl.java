 ...
 *****************************************************************************/
//
// $Id: SVGRadialGradientElementImpl.java,v 1.28 2000/11/22 05:00:14 bella Exp $
//
...
/**
 * SVGRadialGradientElementImpl is the implementation of org.w3c.dom.svg.SVGRadialGradientElement
 */
public class SVGRadialGradientElementImpl extends SVGGradientElementImpl implements
  ...
  public SVGAnimatedLength getFx() {
    if (fx == null) {
      ...
      if (refGradient != null && refGradient instanceof SVGRadialGradientElementImpl) {
        ...
        fx = new SVGAnimatedLengthImpl(new SVGLengthImpl(getCx().getBaseVal().getValueAsString(), this, SVGLengthImpl.X_DIRECTION), this);
      }
    }
    return fx;
  }
  ...
  public SVGAnimatedLength getFy() {
    if (fy == null) {
      ...
      if (refGradient != null && refGradient instanceof SVGRadialGradientElementImpl) {
        ...
        fy = new SVGAnimatedLengthImpl(new SVGLengthImpl(getCy().getBaseVal().getValueAsString(), this, SVGLengthImpl.Y_DIRECTION), this);
      }
    }
    return fy;
  }
  ...
  public Paint getPaint(SVGElementImpl element, float opacity) {
    ...
    for (int i = 0; i < 256; i++) {
      offset = ((double) i) / 255.0;
      Color colAtOffset = getColorAtOffset(offset);
      if (opacity < 1) {
        colAtOffset = new Color(colAtOffset.getRed(), colAtOffset.getGreen(),
                  colAtOffset.getBlue(), (int)(colAtOffset.getAlpha()*opacity));
      }
      colorTable[255 - i] = colAtOffset.getRGB();
    }
    colorModel = new IndexColorModel(8, 256, colorTable, 0, true, -1,
                                         DataBuffer.TYPE_BYTE);

    SVGSVGElement svgElement;
    if (animGradientUnits == SVG_UNIT_TYPE_USERSPACEONUSE) { // use the element's viewport
    ...
    if (animGradientUnits == SVG_UNIT_TYPE_OBJECTBOUNDINGBOX) {
      ...
      // if box is not square, make it square for now and do a scale transform later
  ...
  //                                                + fx + "," + fy + "," + r);
    ...
    BufferedImage  image = new BufferedImage(maxX-minX+1, maxY-minY+1,
    ...
    if (magV1 < (double)1e-10) { // center and focal points are the same
    //  System.out.println("center and focal points are the same");
       ...
       * using the vertical line thru the focus.  Compute the colors
       * by region, starting with the region on the right.  For each
       * region, compute the slope and distance to the circle boundary
       * for each radial.  Use this information to color the interior.
       */
      ...
      /*
       * Traverse top, right, and bottom edges of right region,
       * in clockwise direction.
       */

      for (int x = startX+1; x <= maxX; x++) {
        findBoundaryDistance(x, minY, bdryDists, slopes, index++, cx, cy,
                             fx, fy, r, focalRadius);
      }
      if (startX < maxX) {
        for (int y = minY; y <= maxY; y++) {
          findBoundaryDistance(maxX, y, bdryDists, slopes, index++, cx, cy,
                               fx, fy, r, focalRadius);
        }
      }
      for (int x = maxX; x >= startX+1; x--) {
        findBoundaryDistance(x, maxY, bdryDists, slopes, index++, cx, cy,
                             fx, fy, r, focalRadius);
      }
      ...
      top = minY;
      bottom = maxY;
      scanBlock (new Point(minX, minY), new Rectangle (startX, top,
                       right-startX, bottom-top), bdryDists, slopes, index-1,
                       raster, cx, cy, fx, fy, r, focalRadius, animSpreadMethod);

      index = 0;

      /*
       * Traverse bottom, left, and top edges of left region,
       * in clockwise direction.
       */

      for (int x = startX-1; x >= minX; x--) {
        findBoundaryDistance(x, maxY-1, bdryDists, slopes, index++, cx, cy,
                             fx, fy, r, focalRadius);
      }
      if (startX > minX) {
        for (int y = maxY; y >= minY; y--) {
          findBoundaryDistance(minX, y, bdryDists, slopes, index++, cx, cy,
                               fx, fy, r, focalRadius);
        }
      }
      ...
      left = minX;
      scanBlock (new Point(minX, minY), new Rectangle (left, top,
                       startX-left, bottom-top), bdryDists, slopes, index-1,
                       raster, cx, cy, fx, fy, r, focalRadius, animSpreadMethod);
   ...
   * @param x The x coordinate of a point on the radial.
   * @param y The y coordinate of a point on the radial.
   * @param bdryDists Distances to the boundary circle along line
   * segments (radials) originating at focus and ending at block edges.
   * @param slopes Slopes of the radials generating the bdryDists array.
   * @param index Index of entries to be updated in slopes, bdryDists arrays.
   */
  public void findBoundaryDistance(int x, int y, double[] bdryDists,
                                     double[] slopes, int index,
                                     double cx, double cy, double fx, double fy,
                                     double r, double focalRadius) {
  ...
  /**
   * Determines the color of each image point in a block using boundary
   * point information.  Boundary information is assumed to stored in
   * clockwise direction.
   * @param corner The upper left corner of the region to be scanned.
   * @param block The portion of the region to be scanned.
   * @param bdryDists Distances to the boundary circle along line.
   * segments (radials) originating at focus and ending at block edges.
   * @param slopes Slopes of the radials generating the bdryDists array.
   * @param maxIndex Maximum usable index in slopes, bdryDists arrays.
   */
  public void scanBlock(Point corner, Rectangle block,  double[] bdryDists,
                        double[] slopes, int maxIndex, WritableRaster raster,
                        double cx, double cy, double fx, double fy,
                        double r, double focalRadius, short spreadMethod) {

 ...
 *****************************************************************************/
//
// $Id: SVGSetElementImpl.java,v 1.5 2000/09/27 06:59:15 bella Exp $
//
...
public class SVGSetElementImpl extends SVGAnimationElementImpl implements
  SVGSetElement {
  ...
  //  System.out.println("time: current=" + currentTime + ", start=" + startTime + ", dur=" + duration);
    ...
    if (getAttribute("to").length() > 0) {
      ...
      if (currentTime >= startTime && (repeatForever || (currentTime < (startTime + duration*numRepeats)))) {   // animation is running
        active = true;
        ...
        if (getAttribute("fill").equalsIgnoreCase("remove")) {
          return null;  // this will indicate to use the baseVal as the animVal

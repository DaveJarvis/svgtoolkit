 ...
 *****************************************************************************/
//
// $Id: SvgLoader.java,v 1.8 2000/11/30 23:18:56 bella Exp $
//
  ...
  public void run() {
   ...
   //   System.out.println("parsed " + svgUrl);
      if (running) {
        ...
        if (fragment != null) {
          // will need to change the document in some way so that it represents the correct fragment
          if (fragment.indexOf("svgView") == -1) {
            ...
            if (refElement != null) {
              if (refElement instanceof SVGViewElementImpl) {
                ...
                // set the doc root element to be the nearest ancestor svg elem
                org.w3c.dom.Node parent = view.getParentNode();
                ...
                if (view.getViewBox().getBaseVal().getWidth() > 0) {
                  svgElem.setAttribute("viewBox", ((SVGRectImpl)view.getViewBox().getBaseVal()).toString());
                }
                ...
                if (view.getAttribute("preserveAspectRatio").length() > 0) {
                  svgElem.setAttribute("preserveAspectRatio", view.getAttribute("preserveAspectRatio"));
                }
                if (view.getAttribute("viewTarget").length() > 0) {
                  ...
                  while (st.hasMoreTokens()) {
                    ...
                    org.w3c.dom.Element target = svgDoc.getElementById( viewTargetId);
                ...
                anim.setAttribute("begin", "0"); // this will cause the animation to begin imediately

              } else {
                // simply set the root svg Element to be the nearest ancestor svg elem
                org.w3c.dom.Node parent = refElement.getParentNode();
                while(!(parent instanceof SVGSVGElement)) {
                  parent = parent.getParentNode();
                }
                SVGSVGElement svgElem = (SVGSVGElement)parent;
            ...
            // need to parse the fragment contents and set up the viewBox, zoomAndPan etc of the root svg elem
            ...
            while (st.hasMoreTokens()) {
              ...
              if (attName.equals("viewBox") || attName.equals("preserveAspectRatio")
                ...
                // need to create a g element to do the transform
                ...
                org.w3c.dom.NodeList svgChildren = rootElem.getChildNodes();
                for (int i = 0; i < svgChildren.getLength(); i++) {
                  org.w3c.dom.Node child = svgChildren.item( i);

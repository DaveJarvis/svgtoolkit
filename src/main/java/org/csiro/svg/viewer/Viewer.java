 ...
 *****************************************************************************/
//
// $Id: Viewer.java,v 1.37 2002/03/11 23:46:52 bella Exp $
//
...
public class Viewer extends JFrame implements SvgListener, AbstractView {
   ...
   * @param v  Value to assign to showToolbar.
   */
   ...
   * @param v  Value to assign to showMenu.
   */
  ...
  protected java.net.URL getImage(String name) {
    final var url = getClass().getResource(name);
    if(url == null) {
      throw new MissingResourceException( "Could not load image", getClass().getName(), name );
    }
    return url;
  }
  ...
  private String getParameter( final String key ) {
    System.out.println("Cannot find parameter for: " + key );
    return null;
  }
   ...
   * @param v  Value to assign to showScriptController.
   */
  ...
  public void init() {
    ...
    JMenuItem m94 = makeMenuItem("About", false);
   ...
   // canvas.addMouseMotionListener(scriptMouseHandler);
    ...
    panCursor = Toolkit.getDefaultToolkit().createCustomCursor(
							       (new ImageIcon(getImage("/images/panCursor.gif"))).getImage(),
							       new Point(16,16),
							       "Pan");
    zoomInCursor = Toolkit.getDefaultToolkit().createCustomCursor(
								  (new ImageIcon(getImage("/images/zoomInCursor.gif"))).getImage(),
								  new Point(14,14),
								  "Zoom In");
    zoomOutCursor = Toolkit.getDefaultToolkit().createCustomCursor(
								   (new ImageIcon(getImage("/images/zoomOutCursor.gif"))).getImage(),
								   new Point(14,14),
								   "Zoom Out");
    zoomRectCursor = Toolkit.getDefaultToolkit().createCustomCursor(
								    (new ImageIcon(getImage("/images/zoomRectCursor.gif"))).getImage(),
								    new Point(14,14),
								    "Zoom Out");
...
*/
  ...
  public static void main ( final String[] args ) {
    ...
    viewer.setBounds(20, 20, 750, 750);
    viewer.init();
        ...
        if (showMenu) {
          ...
          for( final MenuElement element : elements ) {
            ...
            if( item instanceof JCheckBoxMenuItem ) {
              if( currentMouseHandler == zoomInMouseHandler &&
                item.getText().equals( "Zoom In" ) ) {
              ...
              else if( currentMouseHandler == panMouseHandler &&
                item.getText().equals( "Pan" ) ) {
                ((JCheckBoxMenuItem) item).setState( true );
              }
              else
                ((JCheckBoxMenuItem) item).setState( currentMouseHandler == linkToMouseHandler &&
                                                       item.getText()
                                                           .equals(
                                                             "Link To" ) );
            }

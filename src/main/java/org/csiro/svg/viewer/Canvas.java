 ...
 *****************************************************************************/
//
// $Id: Canvas.java,v 1.13 2000/11/29 07:03:47 bella Exp $
//
...
public class Canvas extends JComponent
  implements Printable, Pageable, AbstractView {
  ...
  /**
   * Override this function to make sure that the offscreen buffer
   * is cleared when the component is invalidated.
   */
  public void invalidate() {
    // make sure offscreen buffer is cleared when the component is invalidated.
    super.invalidate();
    offscreen = null;
  }

  /**
   * Override update to make sure that the component is not "blanked"
   ...
   * @param v  Value to assign to flipped.
   */
  ...
  public void setBounds(Rectangle r) {
    ...
    if ((theCanvas.width == r.width) &&
        (theCanvas.height == r.height)) return;
  ...
  public void setBounds(int x, int y, int width, int height) {
    ...
    if ((theCanvas.width == width) &&
        (theCanvas.height == height)) return;
  ...
  /**
   * @param g The graphics object that will paint in this region.
   */
  public void paint(Graphics g) {
    ...
    if (width != 0 || height != 0) {
     ...
     //   if (!containsTransparentColours(getGraphic())) {
        //  System.out.println("doesn't contain any transparent colours, doing normal print");
     //     g.setClip(0,0,r.width, r.height);
      //    doPaint(g);
     //   } else {
       //   System.out.println("contains transparent colours, painting to image first");
...
/*  public boolean containsTransparentColours(Graphic graphic) {
    ...
    Paint fillCol = graphic.getFillPaint();
    Paint lineCol = graphic.getLinePaint();
    if (fillCol != null && fillCol instanceof Color) {
        if (((Color)fillCol).getAlpha() < 255)
            return true;
    }
    if (lineCol != null && lineCol instanceof Color) {
        if (((Color)lineCol).getAlpha() < 255)
            return true;
    }
    if (fillCol != null
         && (fillCol instanceof GradientPaint || fillCol instanceof TexturePaint)) {
         // should really do more testing here
        return true;
    }
    if (lineCol != null
         && (lineCol instanceof GradientPaint || lineCol instanceof TexturePaint)) {
         // should really do more testing here
        return true;
    }
    if (graphic instanceof Group) {
      ArrayList elements = ((Group)graphic).getElements();
      int size = elements.size();
      for (int i = 0; i < size; i++) {
        Graphic child = (Graphic) elements.get(i);
        boolean childTransparent = containsTransparentColours(child);
        if (childTransparent) {
          return true;
        }
      }
  ...
  */
  ...
  /**
   * Distributes the MouseEvent to all mouse motion listeners. The MouseEvent is first
   * converted into a WorldMouseEvent which contains world coordinates.
   *
   * @param e The MouseEvent
   */
  ...
  /**
   * Checks that the view has positive width and height.
   * If you don't do this and someone sets the view width
   * and height to be negative, everything is backwards or
   * upside down or both!.
   * <p>
   * It also checks to make sure the aspect is the same as the
   * canvas aspect.
   * <p>
  ...
  private void checkView() {
    ...
    double zoomXratio = (double) theCanvas.width / (double) zoomWidth;
    double zoomYratio = (double) theCanvas.height / (double) zoomHeight;
    ...
    if (zoomXratio > zoomYratio) {
      ...
      theView.width = (double) (theCanvas.width * (theView.height / (double)theCanvas.height));
      theView.x = minX - (double) ((theView.width - zoomWidth) * 0.5);
      ...
      theView.height = (double) (theCanvas.height * (theView.width / (double)theCanvas.width));
      theView.y = minY - (double) ((theView.height - zoomHeight) * 0.5);
  ...
  /**
   * Calculate the world --> screen numbers so I don't have to do
   * the division for every point.
   */
  private void calculateView() {
    screenToWorldX = theView.width / (double)theCanvas.width;
    screenToWorldY = theView.height / (double)theCanvas.height;
    worldToScreenX = theCanvas.width / (double)theView.width;
    worldToScreenY = theCanvas.height / (double)theView.height;
   ...
   * <p>
   * NOTE: The view may be scaled horizontally or vertically to maintain aspect ratio.
   *
   * @param view The new view
   */
  public void setView(Rectangle2D.Double view) {
    if (theView.equals(view)) return;
   ...
   * NOTE: The view may be scaled horizontally or vertically to maintain aspect ratio.
   *
   * @param x The x origin of the new view
   * @param y The y origin of the new view
  ...
  public Rectangle2D.Double getView() {
   ...
   /**
   * Returns the x value of the world coordinate that corresponds to the given canvas
   * x value.
   *
   * @param screenX The x value of the canvas coordinate
  ...
  /**
   * Returns the y value of the world coordinate that corresponds to the given canvas
   * y value.
   *
   * @param screenY The y value of the canvas coordinate
  ...
  /**
   * Returns the x value of the canvas coordinate that corresponds to the given world
   * x value.
   *
   * @param worldX The x value of the world coordinate
  ...
  /**
   * Returns the y value of the canvas coordinate that corresponds to the given world
   * y value.
   *
   * @param worldY The y value of the world coordinate
   ...
   *
   * @param x The amount to pan in the horizontal direction
   * @param y The amount to pan in the vertical direction
   */
   ...
   *
   * @param x The new x origin for the view
   * @param y The new y origin for the view
   */
   ...
   *
   * @param x The x position of the new center
   * @param y The y position of the new center
   */
   ...
   * view will be the center point of the new view. To zoom about a specific point
   * calculate the new view rectangle and use setView.
   * <p>
   * To zoom in on a view use a ratio less than one. To zoom out on a view, use a ratio
   * greater than one.
   *
